...
int busqueda(nodo** p, nodo** pAux,int numB, int tamanioPila);
...
int main()
{
    ...
    cont = busqueda(&p, &pAux, numB, tamanioPila);
    //imprimir2();
    printf("\ncont: %d", cont);
    printf("\nEl numero: %d aparece %d veces,", numB, cont);
    printf("\nEn las posiciones: \n");
    while(pAux != NULL)
    {
        mostrar(desapilar(&pAux));
    }
...
void imprimir(int cantidad) {
    ...
    if (cantidad == 0) {
        ...
        printf("\nLa cantidad de numeros pares mayores al segundo elementos de la pila es: %d\n", cantidad);
    }
